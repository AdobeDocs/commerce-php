...
| `$operations` | array | An array of operation objects. See below for required keys. |
...
**`$operations` array keys:**

- `topic_name` - Must be defined in the `communication.xml` and queue configuration files. It can also be used in one of the Asynchronous API pre-generated topics.
...
- `serialized_data` - An array of serialized data with the following required keys:
  - `entity_id` - Your [entity](https://experienceleague.adobe.com/en/docs/commerce-operations/implementation-playbook/glossary#entity) ID
