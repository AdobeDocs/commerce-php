...
* This may affect existing database structures that use non-standard foreign key configurations
...
1. Set the `restrict_fk_on_non_standard_key` parameter to OFF in their MySQL configuration
2. Use the `--skip-restrict-fk-on-non-standard-key` server option when starting MySQL
